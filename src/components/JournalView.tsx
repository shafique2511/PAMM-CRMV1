...
export function JournalView({
  ...
  trades: Trade[];
  onSyncMT5: () => Promise<{ success: boolean; count: number; error?: string }>;
  ...
  const handleSync = async () => {
    ...
    if (result.success) {
      setSyncStatus({
        message: `Successfully synced ${result.count} trades.`,
        type: "success",
      });
    } else {
      setSyncStatus({
        message: result.error || "Failed to sync trades.",
        type: "error",
      });
    }
  ...
  const filteredTrades = trades.filter((t) => {
    ...
    const matchesFilter =
      filterType === "all" ||
      (filterType === "win" && t.profit > 0) ||
      (filterType === "loss" && t.profit <= 0);
    ...
    if (dateTo)
      matchesDate =
        matchesDate && tradeDate <= new Date(dateTo).getTime() + 86400000;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Professional trade tracking, logic auditing, and advanced metrics.
          </p>
        </div>
        <div className="flex items-center gap-4">
          {syncStatus && (
            <div
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest shadow-sm ${syncStatus.type === "success" ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800" : "bg-rose-50 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400 border border-rose-200 dark:border-rose-800"}`}
            >
          ...
          {!readOnly && (
            ...
            >
              ...
              <span className="text-xs font-black uppercase tracking-widest">
                {isSyncing ? "Syncing..." : "Sync MT5 Data"}
              </span>
            </button>
          )}
        </div>
                ...
                <React.Fragment key={t.id}>
                  ...
                  >
                    ...
                    <td className="px-6 py-5 font-bold text-slate-900 dark:text-slate-300">
                      {Number(t.volume || 0).toFixed(2)} Lots
                    </td>
                    <td className="px-6 py-5 text-right">
                      <span
                        className={`text-sm font-black ${t.profit >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}
                      >
                        {t.profit >= 0 ? "+" : ""}
                        ...
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                              <div className="grid grid-cols-2 gap-4">
                                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 flex flex-col justify-center">
                                  ...
                                  <span className="font-mono text-lg text-slate-900 dark:text-white block font-medium">
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="space-y-1.5">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                  Exit Context (Why Did You Close?)
                                </label>
                                <input
                                  type="text"
                          ...
                          <div className="mt-6 space-y-1.5">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                              Retrospective Notes (Psychology, Mistakes,
                              Lessons)
                            </label>
                            <textarea
                  ...
                  <td colSpan={6} className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <p className="text-xs text-slate-500">
                        Sync with MT5 or adjust filters to view trades.
                      </p>
                    </div>
                  </td>
                </tr>
