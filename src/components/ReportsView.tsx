...
export function ReportsView({
  ...
  const totalFeesCollected = investors.reduce(
    (sum, inv) => sum + inv.feeCollected,
    0,
  );
  const managerWithdrawals = transactions
    .filter((t) => t.type === "manager_withdrawal")
  ...
  const averageAccountSize =
    investors.length > 0 ? totalCapital / investors.length : 0;

  // Group stats for chart
  const groups = investors.reduce(
    (acc, inv) => {
  ...
  const statuses = investors.reduce(
    (acc, inv) => {
  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      const rows = investors.map((inv) => [
        ...
        fromCents(inv.startingCapital),
        fromCents(inv.highWaterMark),
        fromCents(inv.netProfit),
        fromCents(inv.endingCapital),
        fromCents(inv.feeCollected),
        fromCents(inv.unpaidFee),
      ]);
      ...
      link.setAttribute(
        "download",
        `fund_report_${new Date().toISOString().split("T")[0]}.csv`,
      );
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      pdf.save(
        `Fund_Executive_Report_${new Date().toISOString().split("T")[0]}.pdf`,
      );
    } catch (error) {
    ...
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-black tracking-tight bg-gradient-to-r from-blue-600 to-indigo-600 text-transparent bg-clip-text">
              Executive Reports
            </h2>
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
              Fund performance, capital allocation, and business telemetry.
            </p>
          </div>
        </div>
      ...
      <div
        ref={reportRef}
        className="space-y-6 pb-8 bg-slate-50 dark:bg-slate-950 print:bg-white print:p-8 rounded-3xl"
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1">
                ...
                <span className="text-slate-700 dark:text-slate-300 font-bold">
                  {formatCurrency(averageAccountSize)}
                </span>
              </p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 relative overflow-hidden flex flex-col justify-between group">
            ...
            <div>
              <p
                className={`text-3xl font-black tracking-tight ${totalNetProfit >= 0 ? "text-emerald-600 dark:text-emerald-500" : "text-rose-600 dark:text-rose-500"}`}
              >
                {totalNetProfit > 0 ? "+" : ""}
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 relative overflow-hidden flex flex-col justify-between group">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">
                Total High Water Mark
              </h3>
            </div>
            <div>
              <p className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                {formatCurrency(totalHWM)}
              </p>
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col">
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
                Distribution of Assets Under Management across investor groups.
              </p>
            </div>
            {groupData.length > 0 ? (
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col">
            <div className="mb-4">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
                Breakdown of {investors.length} total profiles.
              </p>
            </div>
            {statusData.length > 0 ? (
        ...
        <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-lg font-black tracking-tight text-slate-900 dark:text-white">
                Top Investors Ledger
              </h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
                Highest equity individual profiles and their performance.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {investors
              ...
              .map((inv, i) => (
                <div
                  key={inv.id}
                  className="group flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100/80 dark:hover:bg-slate-800 transition-colors rounded-2xl border border-slate-100 dark:border-slate-700"
                >
                  <div className="flex items-center gap-4 mb-3 sm:mb-0">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="px-2 py-0.5 bg-slate-200/60 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded text-[10px] font-bold uppercase tracking-wider">
                          {inv.group || "Ungrouped"}
                        </span>
                        {inv.status !== "active" && (
                          <span className="px-2 py-0.5 bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 rounded text-[10px] font-bold uppercase tracking-wider">
                            {inv.status}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="grid grid-cols-2 sm:flex sm:items-center gap-6 sm:gap-10 sm:text-right">
                    <div>
                      <span className="block text-[10px] font-black tracking-widest text-slate-400 dark:text-slate-500 uppercase mb-1">
                        AUM Cap
                      </span>
                      ...
                      <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 mt-0.5">
                        {(
                          (inv.startingCapital / (totalCapital || 1)) *
                          100
                        ).toFixed(1)}
                        % of Pool
                      </p>
                    ...
                    <div>
                      ...
                      <p
                        className={`font-black flex items-center sm:justify-end gap-1 ${inv.netProfit >= 0 ? "text-emerald-600 dark:text-emerald-500" : "text-rose-600 dark:text-rose-500"}`}
                      >
                        {inv.netProfit > 0 ? (
                      ...
                      <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 mt-0.5 whitespace-nowrap">
                        Gross: {formatPercent(inv.sharePercentage)}
                      </p>
                    </div>
                  </div>
                </div>
