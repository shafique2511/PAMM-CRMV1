...
import { formatCurrency as globalFormatCurrency, toCents } from "../lib/utils";
import {
  AreaChart,
  Area,
  XAxis,
...
export function InvestorDashboard({
  ...
  const formatCurrency = (value: number) =>
    globalFormatCurrency(value, investor.baseCurrency);
  ...
  const myTransactions = transactions.filter(
    (t) => t.investorId === investor.id,
  );
  const roi =
    investor.startingCapital > 0
      ? (investor.netProfit / investor.startingCapital) * 100
      : 0;
  ...
  const systemGrossLoss = Math.abs(
    losingTrades.reduce((sum, t) => sum + t.profit, 0),
  );
  const systemProfitFactor =
    systemGrossLoss > 0
      ? systemGrossProfit / systemGrossLoss
      : systemGrossProfit > 0
        ? Infinity
        : 0;
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.save(
        `${investor.investorName.replace(/\s+/g, "_")}_Statement_${new Date().toISOString().split("T")[0]}.pdf`,
      );
    } catch (error) {
  ...
  const handleConfirmWithdrawRequest = () => {
    ...
    alert(
      "Withdrawal request submitted successfully. It is now pending manager approval.",
    );
  };
  ...
  const StatCard = ({
    ...
    <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 relative overflow-hidden group">
      <div
        className={`absolute top-0 right-0 w-24 h-24 rounded-full -mr-8 -mt-8 blur-2xl opacity-20 transition-opacity group-hover:opacity-40
      ...
      <div>
        <div className="flex items-end gap-2">
          <h3
            className={`text-3xl font-black tracking-tight ${color === "rose" && trend === "down" ? "text-rose-600 dark:text-rose-500" : color === "emerald" && trend === "up" ? "text-emerald-600 dark:text-emerald-500" : "text-slate-900 dark:text-white"}`}
          >
            {value}
          </h3>
          {trend && (
            <span
              className={`text-xs font-bold mb-1.5 ${trend === "up" ? "text-emerald-500" : trend === "down" ? "text-rose-500" : "text-slate-400"}`}
            >
              {trend === "up" ? "↑" : trend === "down" ? "↓" : "→"}
            </span>
          )}
  ...
  return (
    <div
      className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500"
      ref={dashboardRef}
    >
      <div
      ...
      >
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Real-time performance metrics and asset growth for{" "}
        ...
        >
          ...
          <span className="text-xs font-black uppercase tracking-widest">
            {isGeneratingPDF ? "Generating..." : "Statement PDF"}
          </span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          subtitle={
            distanceToHWM !== 0 && (
              <span
                className={`text-[10px] font-bold uppercase tracking-widest ${distanceToHWM > 0 ? "text-emerald-600" : "text-rose-600"}`}
              >
                Distance: {distanceToHWM > 0 ? "+" : ""}
                {formatCurrency(distanceToHWM)}
              </span>
            )
          }
        />
      ...
      {showTradingJournal && trades.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <StatCard
            ...
            value={
              systemProfitFactor === Infinity
                ? "∞"
                : systemProfitFactor.toFixed(2)
            }
            icon={Activity}
            color="fuchsia"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div
          className={`bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 ${allowWithdrawals ? "lg:col-span-2" : "lg:col-span-3"}`}
        >
            ...
            <div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 p-4 rounded-2xl text-xs font-semibold mb-6 flex gap-2">
                ...
                <span className="font-mono ml-auto">
                  {formatCurrency(investor.endingCapital)}
                </span>
              </div>
              ...
              {myTransactions.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center">
                    <span className="text-xs font-bold text-slate-400 block pb-2">
      ...
      {showConfirmModal && (
        <div
          className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200"
          data-html2canvas-ignore
        >
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 max-w-md w-full mx-4 shadow-xl border border-slate-200 dark:border-slate-800 scale-in-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">
              Please carefully review the details of your withdrawal request.
