...
export function TransactionsView({
  ...
  const filteredAndSortedTransactions = useMemo(() => {
    let result = transactions.filter((t) => {
      ...
      const invName = t.investorId
        ? investors.find((i) => i.id === t.investorId)?.investorName || ""
        : "Manager";
      const matchesSearch =
        invName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (t.notes || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
        (t.referenceId || "").toLowerCase().includes(searchTerm.toLowerCase());

      const matchesInvestor =
        investorFilter === "all" ||
        t.investorId === investorFilter ||
        (investorFilter === "manager" && !t.investorId);
      ...
      if (amountMin)
        matchesAmount =
          matchesAmount && t.amount >= toCents(parseFloat(amountMin));
      if (amountMax)
        matchesAmount =
          matchesAmount && t.amount <= toCents(parseFloat(amountMax));
    ...
    if (sortConfig !== null) {
      result.sort((a, b) => {
        ...
        if (sortConfig.key === "investorName") {
          aValue = a.investorId
            ? investors.find((i) => i.id === a.investorId)?.investorName || ""
            : "Manager";
          bValue = b.investorId
            ? investors.find((i) => i.id === b.investorId)?.investorName || ""
            : "Manager";
        }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 flex items-center gap-4 group hover:border-blue-300 dark:hover:border-blue-800 transition-all">
          ...
          <div>
            ...
            <p className="text-2xl font-black text-slate-900 dark:text-white">
              {formatCurrency(totalFeePayments)}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden relative">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-sm text-slate-400">
                Record a new movement in the system.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="space-y-1.5">
              ...
              >
                ...
                <option value="manager_withdrawal">
                  Manager Withdrawal (Internal)
                </option>
              </select>
            </div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (!isNaN(num)) {
                      ...
                      const managerWithdrawals = managerWithdrawalTxs.reduce(
                        (sum, item) => sum + (Number(item.amount) || 0),
                        0,
                      );
                      const totalFeeCollected = investors.reduce(
                        (sum, item) => sum + (Number(item.feeCollected) || 0),
                        0,
                      );
                      const derivedManagerWalletBalance =
                        totalFeeCollected - managerWithdrawals;
          ...
          <div className="mt-8 pt-6 border-t dark:border-slate-800 flex items-center justify-between">
            <p className="text-xs text-slate-400">
              Ensuring record accuracy protects system integrity.
            </p>
                ...
                >
                  ...
                  <td className="px-4 py-4">
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-900 dark:text-slate-100">
                        {t.investorId
                          ? investors.find((i) => i.id === t.investorId)
                              ?.investorName
                          : "Manager"}
                      </span>
                      {t.method && (
                  ...
                  <td
                    className={`px-4 py-4 font-black ${t.type.includes("withdrawal") ? "text-rose-600" : "text-emerald-600"}`}
                  >
                    <div className="flex flex-col">
                      <span>
                  ...
                  >
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-xs uppercase font-black tracking-widest italic">
                        Archived or Non-existent
                      </p>
                      <p className="text-sm">
                        No recorded transactions match your query.
                      </p>
                    </div>
                  </td>
                </tr>
            ...
            <div className="p-8 space-y-6">
              <div className="flex items-center justify-between pb-4 border-b dark:border-slate-800">
                <div className="flex flex-col">
                  ...
                  <span
                    className={`text-3xl font-black ${selectedTx.type.includes("withdrawal") ? "text-rose-600" : "text-emerald-600"}`}
                  >
              ...
              <div className="grid grid-cols-2 gap-8">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-bold text-slate-900 dark:text-slate-100">
                    {selectedTx.investorId
                      ? investors.find((i) => i.id === selectedTx.investorId)
                          ?.investorName
                      : "Manager"}
                  </p>
                </div>
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-bold text-slate-900 dark:text-slate-100">
                    {selectedTx.method || "Not specified"}
                  </p>
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
