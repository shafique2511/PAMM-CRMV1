      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Manage your identity, security, and account permissions.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-slate-900 dark:text-white">
              {formatCurrency(totalInvestorCapital)}
            </p>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 flex items-center gap-3">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 flex items-center justify-between">
              ...
              {saveMessage && (
                <span className="text-xs font-medium text-emerald-600 dark:text-emerald-400 animate-in fade-in zoom-in">
                  {saveMessage}
                </span>
              )}
            </div>
            ...
            <div className="p-6">
              <div className="max-w-xl">
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
                  Improve your account security by updating your password
                  regularly with a strong combination of characters.
                </p>
                  ...
                  {newPassword && (
                    <div className="space-y-1 animate-in fade-in slide-in-from-top-2">
                      <div className="flex items-center justify-between text-xs mb-1">
                        ...
                        <span
                          className={`font-bold ${evaluatePasswordStrength(newPassword).color.replace("bg-", "text-")}`}
                        >
                        ...
                        {[1, 2, 3, 4].map((step) => (
                          <div
                            ...
                            className={`flex-1 rounded-full transition-all duration-500 ${
                              step <=
                              evaluatePasswordStrength(newPassword).score
            ...
            <div className="p-6">
              <div className="space-y-4">
                {permissionList.map(({ key, label, icon: Icon }) => {
                  const hasAccess =
                    manager?.role === "admin" || (permissions as any)[key];
                  return (
                    <div
                      key={key}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`p-2 rounded-lg ${hasAccess ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400" : "bg-slate-50 text-slate-400 dark:bg-slate-900/50"}`}
                        >
                          <Icon className="w-4 h-4" />
                        </div>
                        <span
                          className={`text-sm ${hasAccess ? "text-slate-700 dark:text-slate-300 font-medium" : "text-slate-400 dark:text-slate-500"}`}
                        >
                          {label}
                        </span>
                      </div>
                ...
                <div className="mt-8 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-xl">
                  <div className="flex gap-3">
                    ...
                    <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                      Your permissions are managed by the System Administrator.
                      Reach out if you need additional access.
                    </p>
                  </div>
                </div>
              )}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        <div className="p-0">
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {auditLogs
              .filter((l) => l.userName === manager?.name)
              .slice(0, 5)
              .map((log) => (
                <div
                  key={log.id}
                  className="p-4 flex items-center gap-4 hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors"
                >
            ...
            {auditLogs.filter((l) => l.userName === manager?.name).length ===
              0 && (
