...
export function DashboardStats({
  ...
  const totalUnpaidFees = investors.reduce(
    (sum, inv) =>
      sum + (Number(inv.unpaidFee) || 0) + (Number(inv.yourFee) || 0),
    0,
  );
  const totalFeeCollected = safeSum(investors, "feeCollected"); // feeCollected natively accrues all-time in the system

  const managerWithdrawalTxs = transactions.filter(
    (t) => t.type === "manager_withdrawal",
  );
  ...
  const historicalNetProfit = history.reduce(
    (sum, h) =>
      sum +
      h.investorSnapshots.reduce(
        (s, inv) => s + (Number(inv.netProfit) || 0),
        0,
      ),
    0,
  );
  ...
  const historicalCashPayouts = history.reduce(
    (sum, h) =>
      sum +
      h.investorSnapshots.reduce((s, inv) => {
        const rebuiltPayout =
          (Number(inv.startingCapital) || 0) +
          (Number(inv.netProfit) || 0) -
          (Number(inv.endingCapital) || 0);
  ...
  const externalWithdrawals = safeSum(
    transactions.filter(
  ...
  const totalInvestorPayoutsAllTime =
    currentCashPayouts + historicalCashPayouts + externalWithdrawals;

  // 3. All-Time Reinvested Profit
  // Reinvestment is simply the profit that was NOT paid out.
  const totalReinvestedAllTime = Math.max(
    0,
    totalProfitGeneratedAllTime - totalInvestorPayoutsAllTime,
  );
  ...
  const totalNetOriginalDeposits = Math.max(
    0,
    totalEndingCapital +
      totalInvestorPayoutsAllTime -
      totalProfitGeneratedAllTime,
  );
  const professionalROI =
    totalNetOriginalDeposits > 0
      ? (totalProfitGeneratedAllTime / totalNetOriginalDeposits) * 100
      : 0;

  // Evaluate highest performer across current state
  const highestPerformingInvestor = [...investors].sort(
    (a, b) => (Number(b.netProfit) || 0) - (Number(a.netProfit) || 0),
  )[0];
  const highestPerformerText =
    highestPerformingInvestor && highestPerformingInvestor.netProfit > 0
  ...
  const capitalHistory = history.map((h) =>
    h.investorSnapshots.reduce((sum, s) => sum + s.endingCapital, 0),
  );
  ...
  const handleWithdraw = () => {
    const amount = toCents(parseFloat(withdrawAmount));
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl p-8 text-white shadow-lg flex flex-col justify-between relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-8">
                  <div>
                    <p className="text-slate-400 font-bold tracking-wider text-xs mb-1 uppercase">
                      Wallet Balance
                    </p>
                    <h2 className="text-4xl md:text-5xl font-black tracking-tight text-white">
                      {formatCurrency(managerWalletBalance)}
                    </h2>
                  </div>
                  <button
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 md:p-5 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-sm">
                  <div>
                    ...
                    <p className="font-bold text-sm md:text-lg text-white truncate">
                      {formatCurrency(managerFeeProfit)}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-400 text-[10px] md:text-xs font-black uppercase tracking-widest mb-1">
                    ...
                    <p className="font-bold text-sm md:text-lg text-cyan-400 truncate">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-sm md:text-lg text-rose-400 truncate">
                      {formatCurrency(managerWithdrawals)}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-400 text-[10px] md:text-xs font-black uppercase tracking-widest mb-1">
                    ...
                    <p className="font-bold text-sm md:text-lg text-amber-400 truncate">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-8">
                  <div>
                    <p className="text-slate-500 dark:text-slate-400 font-bold tracking-wider text-xs mb-1 uppercase">
                      Total Ending Capital
                    </p>
                    <h2 className="text-4xl md:text-5xl font-black tracking-tight text-indigo-600 dark:text-indigo-400">
                      {formatCurrency(totalEndingCapital)}
                    </h2>
                  </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 md:p-5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 rounded-2xl">
                  <div>
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-xs font-black uppercase tracking-widest hidden md:block">
                        Net Profit
                      </p>
                      <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-xs font-black uppercase tracking-widest md:hidden">
                        Profit
                      </p>
                    </div>
                    <p className="font-bold text-sm md:text-lg text-slate-900 dark:text-white truncate">
                      {formatCurrency(totalProfitGeneratedAllTime)}
                    </p>
                  </div>
                  <div>
                    <div className="flex items-center gap-1.5 mb-1">
                      <Repeat className="w-3 h-3 text-blue-500" />
                      <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-xs font-black uppercase tracking-widest">
                        Reinvest
                      </p>
                    </div>
                    <p className="font-bold text-sm md:text-lg text-blue-600 dark:text-blue-400 truncate">
                      {formatCurrency(totalReinvestedAllTime)}
                    </p>
                  </div>
                  <div>
                    <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <p className="font-bold text-sm md:text-lg text-rose-600 dark:text-rose-400 truncate">
                      {formatCurrency(totalInvestorPayoutsAllTime)}
                    </p>
                  </div>
                  <div>
                    <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <p className="font-bold text-sm md:text-lg text-slate-900 dark:text-white truncate">
                      {formatCurrency(totalNetOriginalDeposits)}
                    </p>
                  </div>
                </div>
              </div>
                ...
                {highestPerformingInvestor ? (
                  <>
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight truncate">
                      {highestPerformingInvestor.investorName}
                    </h2>
                    <p className="text-amber-600 dark:text-amber-400 font-bold text-sm sm:text-base mt-0.5">
                      {formatCurrency(highestPerformingInvestor.netProfit)}{" "}
                      generated
                    </p>
                  </>
                ) : (
                  <h2 className="text-2xl font-black text-slate-400 dark:text-slate-600 tracking-tight">
                    N/A
                  </h2>
                )}
              </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {gridStats.map((stat, i) => {
          ...
          return (
            <div
              key={i}
              className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col justify-between hover:border-slate-300 dark:hover:border-slate-600 transition-colors"
            >
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 ${stat.bg} dark:bg-opacity-20`}
              >
                <Icon className={`w-6 h-6 ${stat.color} dark:brightness-150`} />
              </div>
              <div>
                <p
                  className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1 line-clamp-1"
                  title={stat.title}
                >
                  {stat.title}
                </p>
                <h3
                  className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white truncate"
                  title={stat.value}
                >
                  {stat.value}
                </h3>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  disabled={
                    ...
                    toCents(Number(withdrawAmount)) > managerWalletBalance
                  }
