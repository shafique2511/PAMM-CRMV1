...
export function InvoiceModal({ investor, manager, onClose }: InvoiceModalProps) {
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh] print:max-h-none print:shadow-none print:rounded-none">
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-200 print:hidden bg-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900 tracking-tight">Statement Details</h2>
        ...
        <div className="p-12 overflow-y-auto print:p-0 bg-white relative print:overflow-visible" ref={printRef}>
          <div className="relative z-10 max-w-[800px] mx-auto">
            ...
            <div className="flex justify-between items-end border-b-2 border-slate-900 pb-6 mb-8">
              <div>
                <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase uppercase">{manager?.brandName || 'Que PAMM'}</h1>
                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">Investment Management</p>
              </div>
              <div className="text-right">
                <h2 className="text-2xl font-light text-slate-400 uppercase tracking-widest leading-none">Client Statement</h2>
                <div className="mt-4 text-xs font-mono text-slate-600">
                  <span className="text-slate-400 font-sans tracking-wide uppercase text-[10px] mr-2">Ref</span>
                  {invoiceNumber}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-12 mb-12">
              {/* To Section */}
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 border-b border-slate-200 pb-1">Prepared For</p>
                <h3 className="text-lg font-bold text-slate-900 mb-1">{investor.investorName}</h3>
                <div className="text-sm text-slate-600 space-y-0.5">
                  {investor.email && <p>{investor.email}</p>}
                  {investor.phone && <p>{investor.phone}</p>}
                  {investor.country && <p>{investor.country}</p>}
                  {(investor.email || investor.phone || investor.country) && <div className="h-2"></div>}
                  {investor.bankAccount && (
                    <div className="mt-2">
                       <p className="text-[10px] uppercase font-bold text-slate-400">Registered Payment Account:</p>
                       <p className="font-mono text-xs text-slate-900">{investor.bankAccount}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* From/Meta Section */}
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 border-b border-slate-200 pb-1">Statement Details</p>
                <table className="w-full text-sm">
                  <tbody>
                    <tr>
                      <td className="py-1 text-slate-500">Date Issued</td>
                      <td className="py-1 text-right font-medium text-slate-900">{new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</td>
                    </tr>
                    <tr>
                      <td className="py-1 text-slate-500">Base Currency</td>
                      <td className="py-1 text-right font-medium text-slate-900">{displayCurrency}</td>
                    </tr>
                    <tr>
                      <td className="py-1 text-slate-500">Account Group</td>
                      <td className="py-1 text-right font-medium text-slate-900">{investor.group || 'Standard'}</td>
                    </tr>
                    {manager && (
                      <tr>
                        <td className="py-1 text-slate-500">Account Manager</td>
                        <td className="py-1 text-right font-medium text-slate-900">{manager.name}</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            {/* Financial Ledger */}
            <div className="mb-12">
               <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 border-b border-slate-900 pb-1">Account Ledger</p>
               <table className="w-full text-sm">
                 <thead className="border-b border-slate-200 text-slate-500 text-xs text-left">
                   <tr>
                     ...
                     <th className="py-3 font-medium text-right relative">
                       <span className="pr-4 hidden sm:inline text-[10px] absolute right-24 top-4 text-slate-300 font-mono">CCY</span>
                       Amount
                     </th>
                   </tr>
                 </thead>
                 <tbody className="divide-y divide-slate-100">
                    <tr>
                      <td className="py-3 text-slate-900 font-medium">Beginning Account Balance</td>
                      <td className="py-3 text-right font-mono text-slate-900">{formatCurrency(investor.startingCapital, displayCurrency)}</td>
                    </tr>
                    <tr>
                      <td className="py-3 text-slate-900">
                         <div className="flex flex-col">
                            <span>Gross Profit Allocation</span>
                            <span className="text-xs text-slate-400">Share of pool growth ({formatPercent(investor.sharePercentage)})</span>
                         </div>
                      </td>
                    ...
                    <tr>
                      <td className="py-3 text-slate-900">
                         <div className="flex flex-col">
                            ...
                            <span className="text-xs text-slate-400">High Water Mark: {formatCurrency(investor.highWaterMark, displayCurrency)} / Rate: {investor.feePercentage}%</span>
                         </div>
                      </td>
                      <td className="py-3 text-right font-mono text-slate-600">
                    ...
                    <tr className="bg-slate-50 font-bold border-y border-slate-200">
                      <td className="py-4 text-slate-900">Net Profit For Period</td>
            ...
            <div className="grid grid-cols-2 gap-8 mb-8 items-end">
               ...
               <div>
                  <table className="w-full text-sm">
                    <tbody className="divide-y divide-slate-100 border-t border-b border-slate-200">
                       <tr>
                         <td className="py-2 text-slate-500 font-medium text-xs uppercase">Target Reinvestment</td>
                         <td className="py-2 text-right font-mono text-slate-900">{formatCurrency(investor.reinvestAmt, displayCurrency)}</td>
                       </tr>
                       <tr>
                         <td className="py-2 text-slate-500 font-medium text-xs uppercase">Target Cash Payout</td>
                         <td className="py-2 text-right font-mono text-slate-900">{formatCurrency(investor.cashPayout, displayCurrency)}</td>
                       </tr>
                    </tbody>
                  </table>
               </div>
               ...
               <div className="border border-slate-900 p-6 bg-slate-50 text-right">
                 <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Final Asset Value</p>
                 <p className="text-3xl font-mono text-slate-900 tracking-tight">{formatCurrency(investor.endingCapital, displayCurrency)}</p>
               </div>
            </div>
            ...
            <div className="mt-16 pt-6 border-t border-slate-200 text-center">
              <div className="inline-flex flex-col items-center justify-center gap-1">
                 <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Verified Statement</span>
                 <p className="text-xs text-slate-500">This document verifies the period ending ledger values for the account detailed above.</p>
                 {manager?.supportEmail && <p className="text-xs font-mono text-slate-400 mt-2">Support: {manager.supportEmail}</p>}
              </div>
            </div>
