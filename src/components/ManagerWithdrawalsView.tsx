...
export function ManagerWithdrawalsView({
  transactions,
  investors,
  onUpdateStatus,
  readOnly,
  ...
  const processFilter = (txs: Transaction[]) => {
    let result = txs.filter((tx) => {
      ...
      if (dateFrom)
        dateMatch = dateMatch && txTime >= new Date(dateFrom).getTime();
      if (dateTo) {
  ...
  const filteredTxs = processFilter(
    tab === "manager" ? managerWithdrawalTxs : investorWithdrawalTxs,
  );
  const totalWithdrawnManager = managerWithdrawalTxs
    .filter((t) => t.status === "completed")
    .reduce((sum, tx) => sum + tx.amount, 0);
  const pendingInvestorRequests = investorWithdrawalTxs.filter(
    (t) => t.status === "pending",
  ).length;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Review withdrawal mandates and track personal settlements.
          </p>
        </div>
        ...
        {tab === "manager" ? (
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-3xl font-black text-slate-900 dark:text-white">
                {formatCurrency(totalWithdrawnManager)}
              </p>
            </div>
          </div>
        ) : (
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-3xl font-black text-slate-900 dark:text-white">
                {pendingInvestorRequests} Pending
              </p>
            </div>
          </div>
        )}
          ...
          <table className="w-full text-sm text-left border-collapse">
            <thead className="bg-slate-50/50 dark:bg-slate-900/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
              <tr>
                ...
                {tab === "investor" && (
                  <SortHeader label="Beneficiary" sortKey="investorName" />
                )}
                    ...
                    {tab === "investor" && (
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-slate-900 dark:text-slate-100">
                            {inv?.investorName || "Treasury Principal"}
                          </span>
                        </div>
                      </td>
                    )}
                            ...
                            <div className="flex items-center gap-2 pr-2 border-r dark:border-slate-800 mr-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                onClick={() => {
                                  ...
                                  alert(
                                    `Settlement Mandate Approved: Disbursement for ${inv?.investorName} has been recorded.`,
                                  );
                                }}
                  ...
                  >
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm text-slate-400">
                        Your withdrawal ledger is matching zero records for this
                        period.
                      </p>
                    </div>
                  </td>
            ...
            <div className="p-10 space-y-8">
              <div className="flex items-center justify-between pb-8 border-b dark:border-slate-800">
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.15em] shadow-sm
                        ${
              ...
              <div className="grid grid-cols-2 gap-y-8 gap-x-12">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-sm font-bold text-slate-900 dark:text-slate-100">
                    {selectedTx.method || "Internal Transfer"}
                  </p>
                </div>
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 text-slate-400">
              ...
              <div className="pt-6 border-t dark:border-slate-800">
                ...
                <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                  <p className="text-sm text-slate-600 dark:text-slate-400 italic leading-relaxed">
                    {selectedTx.notes ||
                      "No administrative observations recorded for this mandate."}
                  </p>
                </div>
              </div>
            </div>
