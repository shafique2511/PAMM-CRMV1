  ...
  const navGroups = [
    {
      ...
      items: [
        ...
        { id: 'journal', label: 'Trading Journal', icon: BookOpen, show: hasPermission('canSyncMT5', true) },
        { id: 'manager_withdrawals', label: 'Manager Wallet', icon: Briefcase, show: hasPermission('canManageWithdrawals', managerRole !== 'read_only') },
        { id: 'affiliates', label: 'IB Affiliates', icon: Users, show: enableIBModule && hasPermission('canViewAffiliates', true) },
        { id: 'reports', label: 'Reports', icon: PieChart, show: hasPermission('canViewReports', true) },
      ]
    },
    {
      ...
      show: true,
      items: [
        { id: 'dashboard', label: 'My Dashboard', icon: LayoutDashboard, show: !isAdmin },
        { id: 'investors', label: isAdmin ? 'Investor Roster' : 'Statements', icon: Users, show: true },
        { id: 'transactions', label: 'Fund Transfers', icon: ArrowRightLeft, show: isAdmin ? hasPermission('canManageTransactions', true) : true },
        { id: 'journal', label: 'Trading Journal', icon: BookOpen, show: !isAdmin && permissions?.showTradingJournalToInvestors },
      ]
    },
    {
    ...
    <div className="flex flex-col w-64 bg-slate-950 text-slate-300 h-full border-r border-slate-800 shadow-xl overflow-hidden">
      <div className="flex items-center justify-center p-6 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
             <h1 className="text-xl font-black text-white tracking-tight leading-none">Que PAMM</h1>
             <p className="text-[10px] uppercase tracking-widest text-blue-400 font-bold mt-1">Institutional</p>
          </div>
        </div>
      </div>
      
      <div className="flex-1 overflow-y-auto py-6 px-4 space-y-8 scrollbar-hide">
        {navGroups.filter(g => g.show).map((group, idx) => {
          const visibleItems = group.items.filter(item => item.show);
          if (visibleItems.length === 0) return null;
          
          return (
            <div key={idx} className="space-y-2">
              <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-4 mb-3">{group.group}</h2>
